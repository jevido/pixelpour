...
PixelHandler.prototype.addEventHandlers = function () {
  ...
  $(".modal #save").on("click", function () {
    if (Object.keys(lastKeyStroke).length > 0) {
      ...
      ipcRenderer.send(
        "set-keystrokes",
        JSON.stringify({ screen: currentScreen, keyStroke: lastKeyStroke })
      );
      $(".modal").modal("hide");
    }
  });
  ...
  document.addEventListener("keydown", (ev) => {
    ...
    if (currentTime - lastKeyTime > 1000) {
      // 1 seconds for setting a shortcut
      keys = {};
      buffer = {};
      lastKeyStroke = {};
    }
  ...
  ipcRenderer.on("keystroke", (ev, data) => {
    _this.onKeyStroke(ev, data);
  });
        ...
        if (display.id == sources[index].display_id) {
          if (browserwindow == null) {
            ...
            browserwindow.loadURL(
              `file://${__dirname}/assets/js/things/edit/edit.html`
            );
...
PixelHandler.prototype.uploadFile = async function (data, file) {
  ...
  let onFinish = function (uploadID, data) {
    ...
    image.onload = () => {
      $(`.uploaded-files #file-${uploadID} .card-img-top`).attr(
        "style",
...
PixelHandler.prototype.addScreensToScreen = async function () {
  ...
  if (screens) {
    for (let index in screens) {
      ...
      if (enumerate % 3 == 0) {
        // TODO: calculate 3 based on browserwindow width
        // Add a container
...
PixelHandler.prototype.generateCroppedIamge = function () {
  var date = new Date();
  return `Crop ${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(
    date.getDate()
  )} ${pad(date.getHours())}:${pad(date.getMinutes())}.png`;
};
